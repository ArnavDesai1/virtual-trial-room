...
def apply_sprite(image, clothing_path, head_width, head_x, head_y, sprite_type):
    """Apply clothing with MUCH better positioning"""
    try:
        # Load clothing image
        ...
        if sprite_type == 0:  # Shirt/top
            # MUCH better calculations for realistic fitting
            # Calculate proper shoulder width - should be wider than face
            shoulder_width = head_width * 2.5  # Much wider for proper shoulder coverage
            
            # Calculate proper height - from shoulders to waist
            clothing_height = head_width * 2.0  # Proportional height
            
            # Position clothing properly - MUCH LOWER on the body
            # Start at the shoulders (well below the face)
            sprite_x = head_x - head_width * 0.75  # Start much wider to the left
            sprite_y = head_y + head_width * 0.8   # Much further down - at chest level
            
            # Resize sprite to calculated dimensions
            sprite = cv2.resize(sprite, (int(shoulder_width), int(clothing_height)))
        ...
        while run_event.is_set():
            ...
            if len(faces) > 0:
                ...
                if SPRITES[0] and image_path:
                    ...
                    # Calculate and draw shoulder area
                    shoulder_width = w * 2.5
                    shoulder_x = x - w * 0.75
                    shoulder_y = y + w * 0.8  # Much further down - at chest level
                    clothing_height = w * 2.0
                    
                    # Draw shoulder area rectangle
                    cv2.rectangle(image, (int(shoulder_x), int(shoulder_y)), 
                                (int(shoulder_x + shoulder_width), int(shoulder_y + clothing_height)), 
                                (255, 0, 0), 2)
                    
                    # Apply clothing with better positioning
...
if len(sys.argv) > 1:
    ...
    print("Usage: python tryOn_fixed_proper.py <image_path>")
    sys.exit(1)
