...
def calculate_body_from_face(image_shape, face):
    """Calculate body position from face detection - bulletproof version"""
    if face is None:
        return None
...
def apply_clothing_bulletproof(image, clothing_path, body_data):
    """Bulletproof clothing application - handles all edge cases"""
    ...
    try:
        ...
        # Resize clothing to match body with bulletproof scaling
        ...
        # Apply clothing with bulletproof overlay
        return overlay_clothing_bulletproof(image, resized_clothing, (start_x, start_y))
        
    except Exception as e:
        print(f"Clothing application error: {e}")
        return image

def overlay_clothing_bulletproof(body_image, clothing, start_pos):
    """Bulletproof clothing overlay - handles all array indexing safely"""
    try:
        ...
        # Bulletproof alpha blending - check dimensions first
        ...
        while run_event.is_set():
            ...
            if last_body_data:
                if SPRITES[0] and image_path and frame_count % clothing_interval == 0:
                    ...
                    image = apply_clothing_bulletproof(image, image_path, last_body_data)
                else:
...
def main():
    ...
    if len(sys.argv) != 2:
        print("Usage: python tryOn_bulletproof.py <path_to_clothing_image>")
        return
    ...
    # Create GUI
    root = Tk()
    root.title("E-Dressing Room - Bulletproof")
    ...
    # Status label
    status_label = Label(root, text="Initializing bulletproof detection...", font=("Arial", 12))
